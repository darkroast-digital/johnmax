<div id="eventer-events-calendar-container">
    ...
    <div id="calendar-nav" class="calendar-nav-right">
		...
		?>
		<span id="prev-month" class="calendar-nav-btn"><a href="index.php?eventer_year=<?php echo $eventerCalendarVars->prevYear; ?>&eventer_month=<?php  echo $eventerCalendarVars->prevMonth ?>"><?php echo $eventerCalendarVars->monthNames[$eventerCalendarVars->prevMonth - 1].' '.$eventerCalendarVars->prevYear; ?></a></span>
		<?php
			}
		?>
		...
		<?php
			if ($eventerCalendarOptions->showMonthsNavigation == '1') {
		?>
		<span id="next-month" class="calendar-nav-btn"><a href="index.php?eventer_year=<?php echo $eventerCalendarVars->nextYear; ?>&eventer_month=<?php  echo $eventerCalendarVars->nextMonth ?>"><?php echo $eventerCalendarVars->monthNames[$eventerCalendarVars->nextMonth - 1].' '.$eventerCalendarVars->nextYear; ?></a></span>
		<?php
			}
		?>
    </div>
            ...
            if ($m >= ($eventerCalendarVars->weekStartDayID + 1) && $m <= $eventerCalendarVars->numDaysInMonth + $eventerCalendarVars->weekStartDayID) {
                ...
                $dateBoxHTML = '<li class="date-box'. $cssClasses .' date-box-row-'. ($eventer_row + 1) .'" id="date-box-'. $m .'">';
                    ...
                    if ($eventerCalendarOptions->repeatEvents) {
						...
						$query = "SELECT * FROM `eventer_events` WHERE start_date<='$startDate' AND end_date>='$startDate' UNION SELECT * FROM `eventer_events` WHERE  start_date='$startDate'";
                    }
                    else {
                        $query = "SELECT * from eventer_events WHERE start_date='$startDate' AND status='1'";
                    }
