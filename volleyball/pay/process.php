			...
			try {
				...
				switch ( post('txn_type') ) {
					case 'web_accept':
						...
						//update player and team status
						$teamid = isset($data['teamid']) && $data['teamid'] ? substr($data['teamid'], 3) : null;
						$pay_amount = $amount;
						$playerid = isset($data['playerid']) && $data['playerid'] ? $data['playerid'] : null;
						
						if($playerid != ''){
							$playersObj->updateIspaid($playerid);
						}
						
						$teamdetails = $teamObj->fetchteamByid($teamid);
						$totalamount = $teamdetails['totalamount'] + $pay_amount;
						$teamObj->updateTotalamount($totalamount, $teamid);
						
						$balanceamount = $teamdetails['teamcost'] - $totalamount;
						if($balanceamount <= 0){
							$res = $teamObj->updateStatus($teamid);
							if($res){
								$playersObj->updateisPaidbyteamid($teamid);
								$sendemailObj->teamactivationmail($teamdetails);
							}
						}
